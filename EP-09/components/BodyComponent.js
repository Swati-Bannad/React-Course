...
const BodyComponent= () =>{
    ...
    if(onlineStatus==false) return (<div className="container"><h1>Please check internet connection</h1></div>)
                ...
                <div className="res-container">
                    {
                        filteredlistofRestaurants.map( (restaurant)  =>  (
                            <Link to={"/restaurants/" + restaurant.info.id}><Rescard key={restaurant.info.id} resData={restaurant} /></Link> // returning JSX inside map by looping over restraunts
                        ) )
                    }
                </div>
            </div>
